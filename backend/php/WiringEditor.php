...
class WiringEditor {
    ...
    public function saveWiring($name, $working, $language) {
       $fileName = './'.str_replace(' ', '_', $name).'.wiring';
	file_put_contents('./test.txt', print_r(json_decode($working), 1));
	...
	public static function handle($object) {
		...
		try {
			if ($result = @call_user_func_array(array($object,$request['method']),$request['params'])) {
				$response = array ('id' => $request['id'],'result' => $result,'error' => NULL, 'request'=>$request);
			} else {
